...
def get_arrangements(chars, springs):
        ...
        for to_fit in to_fit_dict.keys():
            if char == "?":
                if to_fit + 1 < len(springs_to_fit):
                    new_dict[to_fit + 1] = new_dict.get(to_fit + 1, 0) + to_fit_dict[to_fit]
                if springs_to_fit[to_fit] == ".":
                    new_dict[to_fit] = new_dict.get(to_fit, 0) + to_fit_dict[to_fit]
            elif char == ".":
                if to_fit + 1 < len(springs_to_fit) and springs_to_fit[to_fit + 1] == ".":
                    new_dict[to_fit + 1] = new_dict.get(to_fit + 1, 0) + to_fit_dict[to_fit]
                if springs_to_fit[to_fit] == ".":
                    new_dict[to_fit] = new_dict.get(to_fit, 0) + to_fit_dict[to_fit]
            elif char == "#":
    ...
    return to_fit_dict.get(len(springs_to_fit) - 1, 0) + to_fit_dict.get(len(springs_to_fit) - 2, 0)
...
def get_solution(input):
    ...
    for line in input:
        ...
        springs = [int(spring) for spring in springs.split(",")]
